...
if submit:
    ...
    pdf = pdfkit.from_string(html, False) #trigger download of the pdf after the submit button is pressed; this doesn't work in local computer but in streamlit web
    #pdf = pdfkit.from_string(html, "out.pdf", configuration=config) #when you want to save the pdf file directly and doesn't let you to trigger download in the add
    #pdf = pdfkit.from_string(html,  configuration=config)  #for local hosting of the app where pdfkit package must be specified
    st.balloons()
